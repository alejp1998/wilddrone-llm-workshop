...
try:
    from drone_safari_game import DroneSafariGame  # noqa: F401 -- availability probe
except ImportError:
    print("Note: drone_safari_game.py needs to be in the same directory")

# Default LLM model, overridable via environment (e.g. LLM_MODEL=openai/gpt-4o-mini)
DEFAULT_MODEL = os.environ.get("LLM_MODEL", "gemini/gemini-2.5-flash")
...
class LLMAgent:
    ...
    def __init__(self, model: str = None, tools: Optional[list] = None, system_prompt: str = None):
                ...
                schema = {
                    ...
                    "function": {
                        ...
                        "parameters": {
                            ...
                            "required": [
                                name
                                for name, config in tool.parameters_schema.items()
    ...
    def chat_with_debug(
        self, user_message: str, debug: bool = True, clear_memory: bool = True
    ) -> tuple:
        """
        ...
        try:
            ...
            if not response.choices or len(response.choices) == 0:
                ...
                self.conversation_history.append(
                    {"role": "assistant", "content": empty_response_msg}
                )
            ...
            if hasattr(message, "tool_calls") and message.tool_calls:
                if debug:
                    debug_output.append(
                        f"- Tools called: {[tc.function.name for tc in message.tool_calls]}"
                    )
