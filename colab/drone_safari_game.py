...
class DroneSafariGame:
    ...
    def _setup_grid(self):
        ...
        symbol_to_value = {".": EMPTY, "T": TREE, "Z": ZEBRA, "E": ELEPHANT, "O": ORYX}
    ...
    def _is_adjacent_to_animal(self, row, col):
        ...
        for dr in [-1, 0, 1]:
            for dc in [-1, 0, 1]:
                ...
                if self._is_valid_position(check_row, check_col) and self.grid[
                    check_row, check_col
                ] in [ZEBRA, ELEPHANT, ORYX]:
                    return True
        return False
    ...
    def _get_sensor_summary(self):
        ...
        # Object type names
        object_names = {TREE: "tree", ZEBRA: "zebra", ELEPHANT: "elephant", ORYX: "oryx"}
                    ...
                    if not self._is_valid_position(scan_row, scan_col):
                        ...
                        if boundary_desc:
                            ...
                            proximity_detections.append(
                                f"{' and '.join(boundary_desc)} at {direction_str}"
                            )
                        continue
    ...
    def move(self, move_type, sensors=False):
        ...
        self.message = (
            f"Moved {move_type} to position ({new_row}, {new_col}), facing {self.drone_facing}"
        )
        result = self.message
        if sensors:
            result += "\n" + self._get_sensor_summary()
    ...
    def take_picture(self, sensors=False):
        ...
        if (
            self._is_valid_position(blocking_row, blocking_col)
            and self.grid[blocking_row, blocking_col] == TREE
        ):
    ...
    def get_grid_string(self):
        ...
        symbols = {EMPTY: ".", TREE: "T", ZEBRA: "Z", ELEPHANT: "E", ORYX: "O"}
        ...
        grid_lines.append(
            "   " + "".join([f"{i:2}" for i in range(min(10, GRID_SIZE))])
        )  # Column numbers
    ...
    def visualize(self, figsize=(8, 6)):
        ...
        _im = ax.imshow(vis_grid, cmap=cmap, vmin=0, vmax=0)
                ...
                if cell_value == TREE:
                    ...
                    if tree_icon is None:
                        ...
                        square = patches.Rectangle(
                            (col - 0.4, row - 0.4), 0.8, 0.8, color="brown", zorder=5
                        )
                        ax.add_patch(square)
                    ...
                    if zebra_icon is None:
                        ...
                        square = patches.Rectangle(
                            (col - 0.4, row - 0.4), 0.8, 0.8, color="black", zorder=5
                        )
                        ax.add_patch(square)
                    ...
                    if elephant_icon is None:
                        ...
                        square = patches.Rectangle(
                            (col - 0.4, row - 0.4), 0.8, 0.8, color="gray", zorder=5
                        )
                        ax.add_patch(square)
                    ...
                    if oryx_icon is None:
                        ...
                        square = patches.Rectangle(
                            (col - 0.4, row - 0.4), 0.8, 0.8, color="orange", zorder=5
                        )
                        ax.add_patch(square)
        ...
        for scared_row, scared_col in self.scared_animals:
            ...
            if scared_animal_icon is None:
                ...
                square = patches.Rectangle(
                    (scared_col - 0.4, scared_row - 0.4), 0.8, 0.8, color="red", alpha=0.7, zorder=7
                )
                ax.add_patch(square)
                ax.text(
                    scared_col,
        ...
        for prev_pos, curr_pos in self.movement_trail:
            ...
            ax.plot(
                ...
                alpha=0.7,
                zorder=9,
                label="Movement Trail" if len(self.movement_trail) == 1 else "",
            )
        ...
        for pos, prev_facing, new_facing in self.rotation_trail:
            ...
            for ti in t:
                ...
                curve_row = (
                    (1 - ti) ** 2 * start_row + 2 * (1 - ti) * ti * control_row + ti**2 * end_row
                )
                curve_col = (
                    (1 - ti) ** 2 * start_col + 2 * (1 - ti) * ti * control_col + ti**2 * end_col
                )
            ...
            if drone_visible:
                ...
                if crashed_drone_icon is not None:
                    ...
                    circle = patches.Circle(
                        (drone_col, drone_row), 0.35, color="darkred", zorder=10
                    )
                    ax.add_patch(circle)
        ...
        for (photo_row, photo_col), picture_num in self.photographed_locations.items():
            ...
            if shining_icon is not None:
                ...
                star = patches.Polygon(
                    star_points, facecolor="gold", edgecolor="orange", linewidth=2, zorder=12
                )
                ax.add_patch(star)
...
class InteractiveDroneSafariGame:
    ...
    def update_display(self):
        ...
        # Display the background grid (invisible since it's all white)
        _im = self.ax.imshow(vis_grid, cmap=cmap, vmin=0, vmax=0)
        ...
        if (
            self.game.game_over
            and not self.game.game_won
            and self.game.failure_reason != "scared_animal"
        ):
            # Show crashed drone (but not when animal was scared)
            if drone_visible:
                # Try to use crashed drone icon, fallback to red circle with exclamation
        ...
        box_color = (
            "lightblue"
            if self.game.game_won
            else ("lightgreen" if not self.game.game_over else "lightcoral")
        )
        self.ax.text(
            1.02,
            0.98,
